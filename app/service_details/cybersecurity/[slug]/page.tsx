...
// Get all Cybersecurity slugs for static generation
export async function generateStaticParams() {
    ...
    const cyberSlugs = Object.keys(WEBSITE_DEV_SOLUTIONS_DATA).filter(
        slug =>
            WEBSITE_DEV_SOLUTIONS_DATA[slug].category === 'Cybersecurity' &&
            !hardcodedSlugs.includes(slug)
    );

    return cyberSlugs.map(slug => ({ slug }));
}
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
    ...
    if (!service || service.category !== 'Cybersecurity') {
...
const Page = async ({ params }: { params: Promise<{ slug: string }> }) => {
    ...
    // Only allow Cybersecurity category services
    if (!service || service.category !== 'Cybersecurity') {
        notFound();
    }

    // You can customize the layout based on the service if needed,
    // or use a generic CybersecurityLayout.
    // For now, using the common CybersecurityLayout for all these pages.
